/**
 * DEPARTMENT STUDENT COUNT SCRIPT
 * Shows how many students each department account has processed
 * 
 * Usage: node scripts/department-student-count.js
 */
...
async function getDepartmentStudentCounts() {
    try {
        ...
        // Step 2: Get department email mappings
        console.log('📧 Step 2: Fetching department email mappings...');
        
        const { data: departments, error: deptMappingError } = await supabase
            .from('departments')
            .select('name, email, hod_email');
        ...
        // Step 3: Group by action_by (who processed)
        console.log('📊 Step 3: Analyzing processing data...\n');
        ...
        deptStatus.forEach(record => {
            ...
            const studentName = record.no_dues_forms.student_name || 'Unknown';
            const regNo = record.no_dues_forms.registration_no || 'Unknown';
